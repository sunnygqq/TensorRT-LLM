...
start = None
intermediate = None
def vocabulary(model_path):
    import json
    import os
    tokiner_path = os.path.join(model_path,"tokenizer.json")
    with open(tokiner_path) as f:
        data = json.load(f)
    vocab = data['model']['vocab']
    vocab_dict = {k:v for k,v in vocab.items()}
    start = []
    intermediate = []
    for key in vocab_dict:
        if key[0] == '▁':
            start.append(vocab_dict[key])
        else:
            intermediate.append(vocab_dict[key])
    # print("Len start ",len(start))
    # print("Len intermediate ",len(intermediate))
    
    start = set(start)
    intermediate = set(intermediate)
    return start, intermediate
...
def build_inputs(tokenizer, query, history: List[Tuple[str, str]] = None):
...
def generate(
    ...
    while True:
        ...
        input_ids = build_inputs(tokenizer, input_text, history=history)['input_ids'].int().contiguous().cuda()
        ...
        if streaming:
            for idx, output_ids in enumerate((throttle_generator(output_gen_ids,
                ...
                if 0:
                    ...
                    if output_ids_target[0] in start:
                    ...
                    if ret:
                        ...
                        break
                    output_ids_target_all.append(output_ids_target)
        else:
        ...
        print("\n")
        print("===================================================")
        # print("total_tok is {} and latency is {} and timestamp is {}".format(total_tok,latency,time_stamp))
        print("Speed of Inference: {:.2f} tok/s.".format(float(total_tok)/latency))
        print("===================================================")
        run_i +=1
        print("\n")
        output_ids_target_all_list = flatten_list(output_ids_target_all)
        output_text_all = tokenizer.decode(output_ids_target_all_list)
        history = history + [(input_text, output_text_all)]
...
if __name__ == '__main__':
    ...
    start, intermediate= vocabulary(args.tokenizer_dir)
    generate(**vars(args))
